...
with tab2:
    ...
    st.markdown(
        "Sends each golden question to your **running FastAPI app** (`localhost:8001/query`). "
    ...
    if run_pipeline_btn:
        ...
        def pipeline_cb(i=None, total=None, question="", stage="", response=""):
            if i is None or total is None:
                return
            pct = int((i / total) * 100)
            if stage == "calling":
